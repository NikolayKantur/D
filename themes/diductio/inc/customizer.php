 ...
 */
function diductio_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'diductio_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'diductio_customize_partial_blogdescription',
		) );
	}
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'diductio_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'diductio' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => diductio_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'diductio' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'diductio' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'diductio' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'diductio' ),
		'section'     => 'colors',
	) ) );
...
add_action( 'customize_register', 'diductio_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Twenty Fifteen 1.5
 * @see diductio_customize_register()
 *
 * @return void
 */
function diductio_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function diductio_customize_partial_blogdescription() {
 ...
 *
 * Can be filtered with {@see 'diductio_color_schemes'}.
 *
 ...
 */
function diductio_get_color_schemes() {
	/**
	 ...
	 * }
	 */
	return apply_filters( 'diductio_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'diductio' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'diductio_get_color_scheme' ) ) :
/**
 ...
 */
function diductio_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = diductio_get_color_schemes();
...
endif; // diductio_get_color_scheme

if ( ! function_exists( 'diductio_get_color_scheme_choices' ) ) :
/**
 ...
 */
function diductio_get_color_scheme_choices() {
	$color_schemes                = diductio_get_color_schemes();
...
endif; // diductio_get_color_scheme_choices

if ( ! function_exists( 'diductio_sanitize_color_scheme' ) ) :
/**
 ...
 */
function diductio_sanitize_color_scheme( $value ) {
	$color_schemes = diductio_get_color_scheme_choices();
...
endif; // diductio_sanitize_color_scheme
 ...
 */
function diductio_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = diductio_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = diductio_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = diductio_get_color_scheme_css( $colors );

	wp_add_inline_style( 'diductio-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'diductio_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function diductio_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', diductio_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'diductio_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function diductio_customize_preview_js() {
	wp_enqueue_script( 'diductio-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'diductio_customize_preview_js' );
 ...
 */
function diductio_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function diductio_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-diductio-color-scheme">
		<?php echo diductio_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'diductio_color_scheme_css_template' );
