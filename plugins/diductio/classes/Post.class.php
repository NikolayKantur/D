 ...
 */
class Did_Post extends Diductio
{
    /**
     * @var
     */
    ...
    public function get_passing_info_by_post($user_id, $post_id)
    {
        ...
        if ($row['created_at']) {
            ...
            if (count($passed_lessons) == $lessons_count) {
                $passed_date = array_filter($passed_date);
                // is passing data is exist (checking it because most part of post was passed without this feature)
                
                if(!empty($passed_date)) {
    ...
    function actionEmbedContent()
    {
        ...
        if($host_from_url == $main_host) {
            ...
            if($percent) {
                diductio_view('single-progress', compact('percent'));
            }
        }
    }
    ...
    function get_general_progress($post_id)
    {
        ...
        $posts_users = $st->get_users_by_post($post_id);
    ...
    public static function getCurrentPostFormatBadge() {
        $format_map = Did_Posts::getPostsFormatMap();

        $current_post_format = get_post_format();
        $current_post_format_title = $format_map[$current_post_format][0];
        $current_post_format_color = $format_map[$current_post_format][1];

        return array(
            'title' => $current_post_format_title,
            'color' => $current_post_format_color,
        );
    }

    public static function getTagCategoriesBy($tag_id) {
        $args = array( 
            'tag__in' => $tag_id,
            'posts_per_page' => -1);
        ...
        foreach ($tag_posts as $tag_key => $tag_value) {
            ...
            $tag_categories[$category_info->term_id] = array(
